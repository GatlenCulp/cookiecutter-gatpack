...
OUTPUT_PDF=packet.pdf

# Build Cover Page
rm -f $COVER_LATEX
rm -f $COVER_PDF
gatpack render $COVER_LATEX_TEMPLATE $COVER_LATEX $COMPOSE
gatpack build $COVER_LATEX $COVER_PDF

# Build Further Readings Page
rm -f $FURTHER_READINGS_LATEX
rm -f $FURTHER_READINGS_PDF
gatpack render $FURTHER_READINGS_LATEX_TEMPLATE $FURTHER_READINGS_LATEX $COMPOSE
gatpack build $FURTHER_READINGS_LATEX $FURTHER_READINGS_PDF

# Combine all readings into "packet.pdf"
rm -f $OUTPUT_PDF
gatpack combine $COVER_PDF $READINGS_PDFS $FURTHER_READINGS_PDF $OUTPUT_PDF
